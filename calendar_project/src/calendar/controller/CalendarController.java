...
public class CalendarController implements IController {
  ...
  /**
   * Parses a string into a command and calls appropriate helper with command arguments.
   * @param command string representing command given by user
   */
    ...
    switch (commandScanner.next().toLowerCase()) {
      ...
      case "until":
        this.model.makeEventSeriesUntilDate(subject, weekDays,
              ...
              LocalDate.parse(commandScanner.next()).atTime(23, 59));
        break;
      default:
  ...
  /**
   * Checks that the next word of the scanner is equal to the given string.
   * Throws error otherwise. Advances to next word of scanner
   * @param scanner scanner to check next word is equal to string
   * @param word string to check is equal to next word of scanner
   */
