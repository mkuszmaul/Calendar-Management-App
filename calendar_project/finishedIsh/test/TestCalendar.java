...
public class TestCalendar {
  ...
  public void testMakeEventSeriesUntilDate() {
    ...
    expectedCal.makeEventSeriesUntilDate("Party", tuesdayAndFriday, startTime, endTime, laterTime);
  ...
  public void testEditSeriesSubject() {
    ...
    ArrayList<DayOfWeek> weekDays = new ArrayList<>(Arrays.asList(DayOfWeek.MONDAY, DayOfWeek.WEDNESDAY));

    calendar.makeEventSeriesNTimes("Meeting", weekDays, start, end, 4);

    //initial subjects
    System.out.println("Initial events:");
    for (IEvent event : calendar.getEvents(start, LocalDateTime.of(2024, 12, 31, 23, 59))) {
      System.out.println("Subject: " + event.getSubject());
    }

    //editing whole series
    calendar.editSeriesSubject("Meeting", start, "OOD Meeting", true);
    ...
    calendar.editSeriesSubject("OOD Meeting", secondEventStart, "Changed Meeting", false);

    //print subjects after second update
    System.out.println("\nAfter second update:");
    List<IEvent> events = calendar.getEvents(start, LocalDateTime.of(2024, 12, 31, 23, 59));
    ...
    for (IEvent event : calendar.getEvents(secondEventStart, LocalDateTime.of(2024, 12, 31, 23, 59))) {
  ...
  public void testEditSeriesDateTime() {
    ...
    for (IEvent event : calendar.getEvents(newStart, LocalDateTime.of(2024, 12, 31, 23, 59))) {
  ...
  public void testEditSeriesProperties() {
    ...
    calendar.editSeriesDescription("Meeting", start, "Team Sync", true);
    calendar.editSeriesLocation("Meeting", start, "Conference Room", true);
    calendar.editSeriesStatus("Meeting", start, "true", true);
  ...
  public void testEventSeriesConflicts() {
    ...
    try {
      calendar.makeEventSeriesNTimes("Meeting 2", weekDays, start, end, 4);
    ...
    List<IEvent> allEvents = calendar.getEvents(start, LocalDateTime.of(2024, 12, 31, 23, 59));
  ...
  public void testMakeEventSeriesUntilDateWithConflicts() {
    ...
    LocalDateTime start = LocalDateTime.of(2024, 1, 1, 9, 0);
    LocalDateTime end = LocalDateTime.of(2024, 1, 1, 10, 0);
    LocalDateTime seriesEndDate = LocalDateTime.of(2024, 1, 31, 23, 59);
    ArrayList<DayOfWeek> weekDays = new ArrayList<>(Arrays.asList(DayOfWeek.MONDAY, DayOfWeek.WEDNESDAY));
    ...
    //attempt to create conflicting series
    try {
      calendar.makeEventSeriesUntilDate("Meeting 2", weekDays, start, end, seriesEndDate);
      fail("Should have thrown exception for conflicting series");
    } catch (IllegalArgumentException e) {
      assertEquals("Event series conflicts with existing events", e.getMessage());
    }
    ...
    //non-conflicting series
    ArrayList<DayOfWeek> differentDays = new ArrayList<>(Arrays.asList(DayOfWeek.TUESDAY, DayOfWeek.THURSDAY));
    calendar.makeEventSeriesUntilDate("Meeting 2", differentDays, start, end, seriesEndDate);
